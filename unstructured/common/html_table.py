...
def htmlify_matrix_of_cell_texts(matrix: Sequence[Sequence[str]]) -> str:
    ...
    for row_cell_strs in matrix:
        ...
        for s in row_cell_strs:
            ...
            # Using replace avoids allocating an intermediate list produced by split/join
            s = s.replace("\n", _br)
            # -- normalize whitespace in cell --
            cell_text = _space_join(_split_ws(s))
            # -- emit void `<td/>` when cell text is empty string --
            append(f"<td>{cell_text}</td>" if cell_text else "<td/>")
        append("</tr>")

    append("</table>")
    # Prepend opening tag at the front to match original ordering and avoid extra concatenation

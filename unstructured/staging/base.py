...
def element_to_md(element: Element, exclude_binary_image_data: bool = False) -> str:
    ...
    if isinstance(element, Image):
        if not exclude_binary_image_data and element.metadata.image_base64 is not None:
            if element.metadata.image_mime_type is None:
                return f"![{element.text}](data:image/*;base64,{element.metadata.image_base64})"
            return f"![{element.text}](data:{element.metadata.image_mime_type};base64,{element.metadata.image_base64})"

        if element.metadata.image_url is not None:
            return f"![{element.text}]({element.metadata.image_url})"

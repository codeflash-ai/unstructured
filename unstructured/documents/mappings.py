...
# Precompute the large mapping once to avoid rebuilding it on every function call.
_ONTOLOGY_TO_UNSTRUCTURED_CLASS_MAPPING: Dict[Type[ontology.OntologyElement], Type[Element]] = {
...
def get_ontology_to_unstructured_type_mapping() -> (
    ...
    """
    # Return a shallow copy to preserve the original function's behavior of returning a
    # new dict (so callers can mutate it without affecting the module-level version).
    return _ONTOLOGY_TO_UNSTRUCTURED_CLASS_MAPPING.copy()
